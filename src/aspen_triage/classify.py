...
- `in_progress`: This issue is already being worked on (In Progress, In QA, or has a linked PR). The asker should follow along.
- `resolved_done`: This issue is resolved/done. The asker should verify in the latest release.
- `reported_only`: Same or very similar problem but no one is actively working on it. The asker should comment with details.
- `likely_duplicate`: Essentially the same problem. The asker should add themselves as a watcher or comment there.
...
SYSTEM_PROMPT = (
    "You are a triage assistant for the Aspen Discovery JIRA.\n\n"
    "Given a user's problem description and a list of candidate related issues that were "
...
def _build_candidate_text(results: list[SearchResult]) -> str:
    ...
    for i, r in enumerate(results, start=1):
        ...
        lines.append(
            f"{i}. [{r['jira_key']}] "
            f"(status={r['status']}{resolution}, type={r['issue_type']}) "
            f'"{r["summary"]}"\n'
            f"   Description: {desc}"
        )
    return "\n\n".join(lines)
...
def classify(
    ...
    results = semantic_search(db_path, query, embedding_model, top_k=top_k)
    if not results:
        return [], []
