...
def embed_pending(
    ...
    with connect(db_path) as conn:
        rows = conn.execute(
            "SELECT id, summary, description, embed_text_hash FROM issues"
        ).fetchall()
    ...
    for chunk_start in range(0, total_pending, chunk_size):
        ...
        with connect(db_path) as conn:
            for i, issue_id in enumerate(chunk_ids):
                conn.execute(
                    """
                    UPDATE issues
                    ...
                    WHERE id = ?
                    """,
                    (_serialize_embedding(matrix[i]), embedded_at, chunk_hashes[i], issue_id),
                )
                counts["embedded"] += 1
